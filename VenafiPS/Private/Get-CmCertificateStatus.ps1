...
function Get-CmCertificateStatus {
    <#
    .SYNOPSIS
    ...
    The current use for this is from Get-CmCertificate -IncludeStatus
    ...
    process {
        ...
        $attribs = Get-CmAttribute -Path $Certificate.DN -Attribute 'Disabled', 'Ticket DN' -TrustClient $TrustClient
        $certAttributes = @{
        ...
        if (-not $ticketDN -and -not $stage -and $Certificate.CertificateDetails.RevocationStatus) {
            ...
            if ( $Certificate.CertificateDetails.RevocationInitiatedBy ) {
                $identity = Get-CmIdentity -ID $Certificate.CertificateDetails.RevocationInitiatedBy
        ...
        if ($statusSummary -eq 'Ok' -and $Certificate.Consumers) {
            ...
            $allAttribs = $Certificate.Consumers | Get-CmAttribute -Attribute @('In Error', 'Status', 'Disabled') -TrustClient $TrustClient
