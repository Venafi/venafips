<#
.SYNOPSIS
Remove sensitive information when writing debug info
...
.EXAMPLE
@{'password'='foobar'} | Write-DebugWithSecret
Hide password value from hashtable

.EXAMPLE
$jsonString | Write-DebugWithSecret
Hide value(s) from JSON string

#>
function Write-DebugWithSecret {
    ...
    process {
        ...
        if ( -not $InputObject -or [System.Management.Automation.ActionPreference]::SilentlyContinue -eq $DebugPreference ) {
            return
        }

        Write-Debug ('Write-DebugWithSecret input type: {0}' -f $InputObject.GetType().FullName)
