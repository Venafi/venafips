function Find-TrustObject {
    ...
    To find certificate objects, use Find-TrustCertificate.
    ...
    .EXAMPLE
    Find-TrustObject -Type CertificateInstance

    Get all records

    .EXAMPLE
    Find-TrustObject -Type CertificateInstance -First 50

    Get first 50 records

    .EXAMPLE
    Find-TrustObject -Type ActivityLog -Filter @('activityType', 'eq', 'Notifications') -First 10

    Retrieve 10 records matching the field name

    .EXAMPLE
    Find-TrustObject -Type Certificate -Name searchme

    Get all certificates where the name has 'searchme' in it

    .EXAMPLE
    Find-TrustObject -type Certificate -filter @('selfSigned','eq','True')

    Get all self signed certificates

    .EXAMPLE
    Find-TrustObject -Type ActivityLog -Filter @('activityType', 'eq', 'Notifications') -First 10 -Order @{'activityDate'='desc'}

    Retrieve the most recent 10 records matching the field name

    .EXAMPLE
    Find-TrustObject -Filter @('and', @('activityDate', 'gt', (get-date).AddMonths(-1)), @('or', @('userId', 'eq', 'ab0feb46-8df7-47e7-8da9-f47ab314f26a'), @('userId', 'eq', '933c28de-6352-46f3-bc12-bd96077e8eae')))

    Advanced filtering of results.  This filter will find log entries by 1 of 2 people within the last month.

    .LINK
    https://github.com/Venafi/VenafiPS/blob/main/VenafiPS/Private/Find-TrustObject.ps1
    #>
    ...
    $body = New-TrustSearchQuery @queryParams
