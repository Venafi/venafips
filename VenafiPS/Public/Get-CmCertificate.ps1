function Get-CmCertificate {
    ...
    process {
        ...
        if ( $All ) {
            return (Find-CmCertificate -Path '\ved' -Recursive -TrustClient $TrustClient | Get-CmCertificate -IncludePreviousVersions:$IncludePreviousVersions -ExcludeExpired:$ExcludeExpired -ExcludeRevoked:$ExcludeRevoked -TrustClient $TrustClient)
        }
        ...
        if ( Test-IsGuid($ID) ) {
            ...
            $certs.Add((ConvertTo-CmFullPath -Path $ID))
        }
    }
        ...
        $parallelParams = @{
            ...
            ScriptBlock   = {
                ...
                if ( $PSItem -like '\ved*' ) {
                    ...
                    $thisGuid = [CmObject]::new($PSItem).Guid
                }
                else {
                ...
                if ( $using:IncludeStatus ) {
                    $status = $response | Get-CmCertificateStatus
                    $response | Add-Member @{
