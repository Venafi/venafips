function New-TrustCloudKeystore {
    <#
    .SYNOPSIS
    ...
    .EXAMPLE
    New-TrustCloudKeystore -CloudProvider 'MyGCP' -Name 'MyGCM' -OwnerTeam 'SpecialTeam' -GCM -ProjectID 'woot1'

    Create a new GCM keystore

    .EXAMPLE
    New-TrustCloudKeystore -CloudProvider 'MyAzure' -Name 'MyAKV' -OwnerTeam 'SpecialTeam' -AKV -KeyVaultName 'thisisakeyvault'

    Create a new AKV keystore

    .EXAMPLE
    New-TrustCloudKeystore -CloudProvider 'MyAWS' -Name 'MyACM' -OwnerTeam 'SpecialTeam' -ACM -Region 'us-east-1'

    Create a new ACM keystore

    .EXAMPLE
    New-TrustCloudKeystore -CloudProvider 'MyAWS' -Name 'MyACM' -OwnerTeam 'SpecialTeam' -ACM -Region 'us-east-1' -IncludeExpiredCertificates

    Create a new keystore and include expired certificates during discovery

    .EXAMPLE
    New-TrustCloudKeystore -CloudProvider 'MyAWS' -Name 'MyACM' -OwnerTeam 'SpecialTeam' -ACM -Region 'us-east-1' -PassThru
    ...
    process {
        ...
        if ( $PSCmdlet.ShouldProcess($Name, ('Create {0} cloud keystore' -f $variables.input.type)) ) {
            ...
            $response = Invoke-TrustGraphQL -Query $query -Variables $variables | Select-Object -ExpandProperty createCloudKeystore
