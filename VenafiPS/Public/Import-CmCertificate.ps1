function Import-CmCertificate {
    ...
    process {
        ...
        $fullPolicyPath = $PolicyPath | ConvertTo-CmFullPath
        if ( -not (Test-CmObject -Path $fullPolicyPath -ExistOnly -TrustClient $TrustClient) ) {
            if ( $Force ) {
                Write-Verbose "Creating policy path $fullPolicyPath"
                New-CmPolicy -Path $fullPolicyPath -Force -TrustClient $TrustClient
            }
            else {
        ...
        $parallelParams = @{
            ...
            ScriptBlock   = {
                ...
                try {
                    ...
                    if ( $using:PassThru ) {
                        Get-CmObject -Guid $response.Guid.trim('{}')
                    }
                }
                catch {
