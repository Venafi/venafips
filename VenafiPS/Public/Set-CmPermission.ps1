function Set-CmPermission {
    ...
    The id that represents the user or group.  You can use Find-CmIdentity or Get-CmPermission to get the id.
    ...
    .LINK
    https://venafi.github.io/VenafiPS/functions/Set-CmPermission/

    .LINK
    https://github.com/Venafi/VenafiPS/blob/main/VenafiPS/Public/Set-CmPermission.ps1
    ...
    process {
        ...
        if ( $Path ) {
            $thisGuid = [CmObject]::new($Path).Guid
        }
        else {
            $thisGuid = $Guid
        }
        ...
        if ( $PSCmdlet.ParameterSetName -like 'PermissionObject*' ) {
            ...
            $thisPerm = $thisGuid | Get-CmPermission -IdentityId $IdentityId -Explicit | Select-Object -ExpandProperty ExplicitPermissions
