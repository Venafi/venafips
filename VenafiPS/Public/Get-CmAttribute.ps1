function Get-CmAttribute {
    ...
    .EXAMPLE
    Get-CmAttribute -Path 'certs' -Attribute 'Contact' -AsValue
    ...
    begin {
        ...
        if ( $Class -and $All ) {
            ...
            $classAttributes.$Class = Get-CmClassAttribute -ClassName $Class -TrustClient $TrustClient | Select-Object -ExpandProperty Name -Unique
        }
    }
    ...
    process {
        ...
        $attrib = if ( $All ) {
            if ( $Class ) {
                ...
                $thisObject = Get-CmObject -Path $Path -TrustClient $TrustClient

                $thisType = $thisObject.TypeName

                if ( -not $classAttributes.$thisType ) {
                    Write-Verbose ('getting attributes for {0}' -f $thisType)
                    $classAttributes.$thisType = Get-CmClassAttribute -ClassName $thisObject.TypeName -TrustClient $TrustClient | Select-Object -ExpandProperty Name -Unique
                }
