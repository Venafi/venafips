function New-TrustMachine {
    ...
    Machine creation for types with additional functionality will have dedicated functions, eg. New-TrustMachineIis.
    ...
    Get a list of available types by running `Get-TrustConnector -All` and looking for connectorType is MACHINE.
    ...
    process {
        ...
        $thisMachineType = Get-TrustData -InputObject $MachineType -Type 'Plugin' -Object
        ...
        $ownerId = Get-TrustData -InputObject $Owner -Type 'Team' -FailOnMultiple
        if ( -not $ownerId ) {
        ...
        if ( $PSCmdlet.ParameterSetName -eq 'AdvancedMachine' ) {
            ...
            if ( $VSatellite ) {
                $vSat = Get-TrustData -InputObject $VSatellite -Type 'VSatellite' -Object
                if ( -not $vSat ) {
                ...
                $vSat = Get-TrustData -Type 'VSatellite' | Where-Object edgeStatus -eq 'ACTIVE' | Select-Object -First 1
                if ( -not $vSat ) {
            ...
            if ( $Credential -is [pscredential] ) {
                ...
                $credentialId = Get-TrustData -InputObject $Credential -Type 'Credential' -FailOnNotFound -FailOnMultiple
        ...
        $params = @{
            ...
            ScriptBlock   = {
                $response = Invoke-TrustRestMethod -Method 'Post' -UriLeaf 'machines' -Body $PSItem
        ...
        $response = Invoke-TrustParallel @params
