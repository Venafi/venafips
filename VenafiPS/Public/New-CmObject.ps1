function New-CmObject {
    ...
    $newPath = $Path | ConvertTo-CmFullPath
    ...
    if ( $SourcePath ) {
        $fullSourcePath = $SourcePath | ConvertTo-CmFullPath

        $o = Get-CmObject -Path $fullSourcePath
        $oa = $o | Get-CmAttribute -All
    ...
    if ( $PSCmdlet.ShouldProcess($newPath, ('Create {0} Object' -f $params.Body.Class)) ) {
        ...
        do {
            ...
            switch ($response.Result) {
                ...
                1 {
                    ...
                    $returnObject = [CmObject]::new($response.Object.DN, $response.Object.Guid, $response.Object.TypeName)
                }
                    ...
                    if ( $Class -in 'Policy', 'Device' ) {
                        if ( -not $Force ) {
                            ...
                            for ($i = 3; $i -lt ($pathSplit.Count - 1); $i++) {
                                if ( -not (Find-CmObject -Path ($pathSplit[0..($i - 1)] -join '\') -Pattern $pathSplit[$i])) {
                                    Write-Verbose ('Creating missing policy folder {0}' -f ($pathSplit[0..$i] -join '\'))
                                    New-CmPolicy -Path ($pathSplit[0..$i] -join '\')
                                }
                            }
        ...
        if ( $Attribute.Certificate ) {
            ...
            Add-CmCertificateAssociation @associateParams
        }
