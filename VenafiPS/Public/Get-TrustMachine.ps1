function Get-TrustMachine {
    ...
    .EXAMPLE
    Get-TrustMachine -Machine 'ca7ff555-88d2-4bfc-9efa-2630ac44c1f2'
    ...
    .EXAMPLE
    Get-TrustMachine -All -IncludeConnectionDetails
    ...
    process {
        ...
        if ( $PSCmdlet.ParameterSetName -eq 'All' ) {
            ...
            $allMachines = Find-TrustObject -Type Machine -TrustClient $TrustClient

            if ( $IncludeConnectionDetail ) {
                $params = @{
                    InputObject   = $allMachines
                    ScriptBlock   = { $PSItem | Get-TrustMachine }
            ...
            $mId = Get-TrustData -Type Machine -InputObject $Machine
            if ( $mId ) {
                try {
