function New-TrustCloudProvider {
    <#
    .SYNOPSIS
    ...
    begin {
        ...
        $ownerId = Get-TrustData -Type Team -InputObject $OwnerTeam
    ...
    process {
        ...
        if ( $PSBoundParameters.ContainsKey('AuthorizedTeam') ) {
            $variables.input.authorizedTeams = @($AuthorizedTeam | Get-TrustData -Type Team)
        }
        ...
        if ( $PSCmdlet.ShouldProcess($Name, ('Create {0} cloud provider' -f $variables.input.type)) ) {
            ...
            $response = Invoke-TrustGraphQL -Query $query -Variables $variables | Select-Object -ExpandProperty createCloudProvider

            if ( $Validate ) {
                $validateResponse = Invoke-TrustGraphQL -Query $validateQuery -Variables @{'cloudProviderId' = $response.cloudProviderId }
                $response | Add-Member @{
