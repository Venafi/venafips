function Set-CmWorkflowTicketStatus {
    <#
    .SYNOPSIS
    ...
    .LINK
    https://venafi.github.io/VenafiPS/functions/Set-CmWorkflowTicketStatus/

    .LINK
    https://github.com/Venafi/VenafiPS/blob/main/VenafiPS/Public/Set-CmWorkflowTicketStatus.ps1
    ...
    [Alias('Set-VdcWorkflowTicketStatus')]
    ...
    process {
        ...
        foreach ($thisGuid in $TicketGuid) {
            ...
            if ( $PSCmdlet.ShouldProcess($params.Body.GUID, 'Set workflow ticket status') ) {
                ...
                if ( -not ($response.Result -eq [CmWorkflowResult]::Success) ) {
                    throw ("Error setting workflow ticket status, error is {0}" -f [enum]::GetName([CmWorkflowResult], $response.Result))
                }
            }
        }
    }
