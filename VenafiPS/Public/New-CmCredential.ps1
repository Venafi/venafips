function New-CmCredential {
    ...
    process {
        ...
        if ( $CertificatePath ) {
            ...
            # validate we have the correct password now as CM will only do this when trying to retrieve and not on creation
            try {
        ...
        if ( $PSCmdlet.ShouldProcess($newPath, 'Create credential') ) {
            ...
            if ( $PassThru ) {
                Get-CmObject -Path $newPath -TrustClient $TrustClient
            }
        }
    }
}
