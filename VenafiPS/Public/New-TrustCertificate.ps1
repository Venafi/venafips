        ...
        switch ($PSCmdlet.ParameterSetName) {
            ...
            'ExistingCSR' {
                ...
                if ( Test-IsGuid($Certificate) ) {
                    ...
                    $findCerts = Find-TrustCertificate -Name $Certificate -VersionType CURRENT -Status ACTIVE
                    if ( $findCerts ) {
                        $c = @($findCerts).Count
                        if ($c -gt 1) {
            ...
            try {
                ...
                if ( $Wait ) {
                    ...
                    while ( $status -notin $terminalStatuses ) {
                        ...
                        $certRequest = Get-TrustCertificateRequest -CertificateRequest $certRequestId
                        $status = $certRequest.status
                        Write-Verbose "Current certificate request status: $status"
                    }
                }
                ...
                if ( $PassThru ) {
                    ...
                    # we need to remove certificateRequestId and add it back since the new cert request has 'id' but Get-TrustCertificateRequest returns 'certificateRequestId' when -Wait is used
                    ...
                    if ( $certRequest.certificateIds ) {
                        ...
                        $actualCert = Get-TrustCertificate -CertificateId $certRequest.certificateIds[0]
                        $out | Add-Member @{
