function Get-TrustIssuingTemplate {
    <#
    .SYNOPSIS
    ...
    begin {
        if ( $CertificateAuthority ) {
            $caId = Get-TrustData -InputObject $CertificateAuthority -Type 'CertificateAuthority' -FailOnNotFound
        }
    }
    ...
    process {
        ...
        if ( $PSBoundParameters.ContainsKey('IssuingTemplate') ) {
            if ( Test-IsGuid($IssuingTemplate) ) {
                ...
                # search by name
                return Get-TrustIssuingTemplate -All | Where-Object { $_.name -eq $IssuingTemplate }
            }
        }
