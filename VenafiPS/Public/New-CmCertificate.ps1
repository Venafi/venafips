function New-CmCertificate {
    ...
    This property is useful if you are wanting to immediately access the certificate when New-CmCertificate is finished, eg. to pipe to Export-CmCertificate.
    ...
    .LINK
    https://venafi.github.io/VenafiPS/functions/New-CmCertificate/

    .LINK
    https://github.com/Venafi/VenafiPS/blob/main/VenafiPS/Public/New-CmCertificate.ps1
    ...
    param (
        ...
        [Parameter()]
        [CmManagementType] $ManagementType,
    ...
    begin {
        ...
        if ( $PSBoundParameters.ContainsKey('ManagementType') ) {
            $params.Body.Add('ManagementType', [enum]::GetName([CmManagementType], $ManagementType))
        }
    ...
    process {
        ...
        if ( $PSCmdlet.ShouldProcess("$Path\$Name", 'Create new certificate') ) {
            ...
            try {
                ...
                if ( $PassThru ) {
                    $newCert = Get-CmObject -Path $response.CertificateDN
