function Find-TrustCertificateRequest {
    <#
    .SYNOPSIS
    ...
    .EXAMPLE
    Find-TrustCertificateRequest

    Get all certificate requests

    .EXAMPLE
    Find-TrustCertificateRequest -Application 'MyApp' -CreateDateFrom (Get-Date).AddDays(-7)
    ...
    [Alias('Find-VcCertificateRequest')]
    ...
    if ( $PSCmdlet.ParameterSetName -eq 'AdvancedFilter' ) {
        ...
        switch ($PSBoundParameters.Keys) {
            ...
            'Application' { $null = $newFilter.Add(@('applicationId', 'IN', @(($Application | Get-TrustData -Type Application)))) }
            'User' { $null = $newFilter.Add(@('certificateOwnerUserId', 'MATCH', @(($User | Get-TrustData -Type User)))) }
            'IssuingTemplate' { $null = $newFilter.Add(@('certificateIssuingTemplateId', 'MATCH', @(($IssuingTemplate | Get-TrustData -Type IssuingTemplate)))) }
