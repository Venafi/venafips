function Get-CmPermission {
    ...
    CmObject representing an object in Certificate Manager, Self-Hosted, eg. from Find-CmObject or Get-CmObject
    ...
    You can use Find-CmIdentity to search for identities.
    ...
    .EXAMPLE
    Get-CmPermission -Path '\VED\Policy\barron'
    ...
    IdentityPath        : CN=Greg Brownstein,OU=Users,OU=Enterprise Administration,DC=domain,DC=net
    IdentityName        : greg
    ExplicitPermissions : CmPermission
    ImplicitPermissions : CmPermission

    Get explicit and implicit permissions for users/groups on a specific policy folder

    .EXAMPLE
    Find-CmObject -Path '\VED' -Recursive | Get-CmPermission -IdentityId 'AD+myprov:jasdf87s9dfsdfhkashfg78f7'

    Find assigned permissions for a specific user across all objects

    .LINK
    https://venafi.github.io/VenafiPS/functions/Get-CmPermission/

    .LINK
    https://github.com/Venafi/VenafiPS/blob/main/VenafiPS/Public/Get-CmPermission.ps1

    .LINK
    https://github.com/Venafi/VenafiPS/blob/main/VenafiPS/Public/Get-CmIdentityAttribute.ps1
    ...
    process {
        ...
        foreach ( $thisInputObject in $newInputObject ) {
            ...
            switch ( $PsCmdLet.ParameterSetName) {
                'ByObject' {
                    $thisCmObject = $thisInputObject
                }

                Default {
                    $thisCmObject = [cmObject]::new($thisInputObject)
                }
            }

            $uriBase = ('Permissions/Object/{{{0}}}' -f $thisCmObject.Guid )
            ...
            try {
                ...
                Write-Error ('Couldn''t obtain list of permissions for {0}.  {1}' -f $thisCmObject.Path, $_ | Out-String)
                continue
            }
            ...
            foreach ( $thisId in $identities ) {
                ...
                Write-Verbose ('Path: {0}, Id: {1}' -f $thisCmObject.Path, $thisId)
                ...
                $thisReturnObject = [PSCustomObject] @{
                    Path         = $thisCmObject.Path
                    Guid         = $thisCmObject.Guid
                    Name         = $thisCmObject.Name
                    TypeName     = $thisCmObject.TypeName
                    IdentityId   = $thisId
                    IdentityPath = $null
                    IdentityName = $null
                }
                ...
                try {
                    ...
                    if ( $Explicit ) {
                        $thisReturnObject.ExplicitPermissions = [CmPermission] $response.ExplicitPermissions
                        $thisReturnObject.ImplicitPermissions = [CmPermission] $response.ImplicitPermissions
                    }
                    else {
                        $thisReturnObject.EffectivePermissions = [CmPermission] $response.EffectivePermissions
                    }
                    ...
                    $attribResponse = Get-CmIdentityAttribute @attribParams -ErrorAction SilentlyContinue
                    ...
                    if ( $_ -like '*Unable to verify principal*' ) {
                        if ( $Explicit ) {
                            ...
                            if ( $notFoundResponse.Permissions ) {
                                $thisReturnObject.ExplicitPermissions = [CmPermission]$notFoundResponse.Permissions
                            }
                        }
                    }
                    else {
                        Write-Error ('Unable to retrieve permissions.  Path: {0}, Id: {1}, Error: {2}' -f $thisCmObject.Path, $thisId, $_)
                    }
                }
