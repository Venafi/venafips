function Export-TrustReport {
    ...
    .EXAMPLE
    $reportData = Export-TrustReport -Report 'Custom expiration report'

    Get report data for further processing

    .EXAMPLE
    Export-TrustReport -Report 'Custom expiration report' -OutPath '~/reports'
    ...
    process {
        ...
        $reportId = if ( Test-IsGuid($Report) ) {
            ...
            $response = Invoke-TrustGraphQL -Query $query -Variables $variables
        ...
        $getReportResponse = Invoke-TrustGraphQL -Query $getReportQuery -Variables $getReportVars
