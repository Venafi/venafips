class VdcObject {
    ...
    VdcObject([string] $Path, [guid] $Guid, [string] $TypeName) {
        $this._init($Path, $Guid, $TypeName)
    }

    # Construct from Path — resolves Guid and TypeName via API
    VdcObject([string] $Path) {
        $response = Invoke-TrustRestMethod -Method Post -UriLeaf 'config/DnToGuid' -Body @{ ObjectDN = ($Path | ConvertTo-VdcFullPath) }
