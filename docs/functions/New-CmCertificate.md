...
### ByName (Default)
```
New-CmCertificate -Path <String> -Name <String> [-CommonName <String>] [-Csr <String>]
 [-CertificateType <String>] [-CertificateAuthorityPath <String>] [-CertificateAuthorityAttribute <Hashtable>]
 [-ManagementType <CmManagementType>] [-SubjectAltName <Hashtable[]>] [-CustomField <Hashtable>] [-NoWorkToDo]
 [-Device <Hashtable[]>] [-TimeoutSec <Int32>] [-PassThru] [-TrustClient <TrustClient>]
...
### EXAMPLE 1
```
New-CmCertificate -Path '\ved\policy\folder' -Name 'mycert.com'
Create certificate by name.  A CA template policy must be defined.
```

### EXAMPLE 2
```
New-CmCertificate -Path '\ved\policy\folder' -Name 'mycert.com' -CertificateAuthorityPath '\ved\policy\CA Templates\my template'
Create certificate by name with specific CA template
```

### EXAMPLE 3
```
New-CmCertificate -Path '\ved\policy\folder' -CertificateAuthorityPath '\ved\policy\CA Templates\my template' -Csr '-----BEGIN CERTIFICATE REQUEST-----\nMIIDJDCCAgwCAQAw...-----END CERTIFICATE REQUEST-----'
Create certificate using a CSR
```

### EXAMPLE 4
```
New-CmCertificate -Path '\ved\policy\folder' -Name 'mycert.com' -CertificateAuthorityPath '\ved\policy\CA Templates\my template' -CustomField @{''=''}
Create certificate and update custom fields
```

### EXAMPLE 5
```
New-CmCertificate -Path '\ved\policy\folder' -CommonName 'mycert.com' -CertificateAuthorityPath '\ved\policy\CA Templates\my template' -PassThru
Create certificate using common name.  Return the created object.
```

### EXAMPLE 6
```
New-CmCertificate -Path '\ved\policy\folder' -Name 'mycert.com' -CertificateAuthorityPath '\ved\policy\CA Templates\my template' -SubjectAltName @{'Email'='me@x.com'},@{'IPAddress'='1.2.3.4'}
Create certificate including subject alternate names
```

### EXAMPLE 7
```
New-CmCertificate -Path '\ved\policy\folder' -Name 'mycert.com' -Device @{'PolicyDN'=$DevicePath; 'ObjectName'='MyDevice'; 'Host'='1.2.3.4'} -Application @{'DeviceName'='MyDevice'; 'ObjectName'='BasicApp'; 'DriverName'='appbasic'}
Create a new certificate with associated device and app objects
```

### EXAMPLE 8
```
New-CmCertificate -Path '\ved\policy\certs' -Name 'www.barron.com' -CertificateAuthorityPath '\ved\policy\CA Templates\my template' -TimeoutSec 30 -PassThru | Export-CmCertificate -PKCS12 -PrivateKeyPassword 'mySecretPassword!'
```
...
This property is useful if you are wanting to immediately access the certificate when New-CmCertificate is finished, eg.
to pipe to Export-CmCertificate.
...
[https://venafi.github.io/VenafiPS/functions/New-CmCertificate/](https://venafi.github.io/VenafiPS/functions/New-CmCertificate/)

[https://github.com/Venafi/VenafiPS/blob/main/VenafiPS/Public/New-CmCertificate.ps1](https://github.com/Venafi/VenafiPS/blob/main/VenafiPS/Public/New-CmCertificate.ps1)
