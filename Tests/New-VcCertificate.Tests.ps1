...
Describe "New-VcCertificate" -Tags 'Unit' {
    ...
    BeforeEach {
        ...
        Mock -CommandName 'Invoke-VenafiRestMethod' -MockWith { $mockCertRequestResponse } -ModuleName $ModuleName
    }
    ...
    Context "PassThru" {
        ...
        It "Should include certificate when certificateIds is present" {
            Mock -CommandName 'Invoke-VenafiRestMethod' -MockWith { $mockCertRequestIssuedResponse } -ModuleName $ModuleName
    ...
    Context "Wait - terminal status immediately" {
        ...
        It "Should not poll when initial status is FAILED" {
            Mock -CommandName 'Invoke-VenafiRestMethod' -MockWith {
                [pscustomobject]@{
                    certificateRequests = [pscustomobject]@{
                        id     = $certRequestId
                        status = 'FAILED'
    ...
    Context "API errors" {
        ...
        It "Should write error when API call fails" {
            Mock -CommandName 'Invoke-VenafiRestMethod' -MockWith { throw 'API error' } -ModuleName $ModuleName
