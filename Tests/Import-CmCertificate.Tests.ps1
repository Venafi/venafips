...
Describe 'Import-CmCertificate' -Tags 'Unit' {

    BeforeEach {
        Mock -CommandName 'ConvertTo-CmFullPath' -MockWith { $testPolicyPath } -ModuleName $ModuleName
        Mock -CommandName 'Test-CmObject' -MockWith { $true } -ModuleName $ModuleName
        ...
        Mock -CommandName 'Get-CmObject' -MockWith { $mockCmObject } -ModuleName $ModuleName
    }

    Context 'PKCS12 data import via -Data' {

        It 'Should pass PKCS12 data directly as CertificateData' {
            Import-CmCertificate -PolicyPath $testPolicyPath -Data $testPkcs12Data
    ...
    Context 'Base64-encoded PEM cert-only import' {
        ...
        It 'Should pass base64-encoded PEM to Invoke-TrustParallel' {
            Import-CmCertificate -PolicyPath $testPolicyPath -Data $testCertPemBase64
            Should -Invoke -CommandName 'Invoke-TrustParallel' -Times 1 -ModuleName $ModuleName -ParameterFilter {
    ...
    Context 'Base64-encoded PEM cert+key (PKCS8) import' {
        ...
        It 'Should pass full cert+key base64 data to Invoke-TrustParallel' {
            Import-CmCertificate -PolicyPath $testPolicyPath -Data $testCertAndKeyPemBase64 -PrivateKeyPassword 'pass'
            Should -Invoke -CommandName 'Invoke-TrustParallel' -Times 1 -ModuleName $ModuleName -ParameterFilter {
    ...
    Context 'Separate PrivateKey parameter' {
        ...
        It 'Should set PrivateKeyData in the body when -PrivateKey is provided' {
            ...
            Import-CmCertificate -PolicyPath $testPolicyPath -Data $testCertPemBase64 -PrivateKey $testKeyData -PrivateKeyPassword 'pass'
            Should -Invoke -CommandName 'Invoke-TrustParallel' -Times 1 -ModuleName $ModuleName -ParameterFilter {
    ...
    Context 'PrivateKeyPassword handling' {
        ...
        It 'Should set Password in body when PrivateKeyPassword provided as string' {
            Import-CmCertificate -PolicyPath $testPolicyPath -Data $testPkcs12Data -PrivateKeyPassword 'myPassword!'
        ...
        It 'Should accept SecureString for PrivateKeyPassword' {
            ...
            Import-CmCertificate -PolicyPath $testPolicyPath -Data $testPkcs12Data -PrivateKeyPassword $secPass
            Should -Invoke -CommandName 'ConvertTo-PlaintextString' -Times 1 -ModuleName $ModuleName
        }
        ...
        It 'Should accept PSCredential for PrivateKeyPassword' {
            ...
            Import-CmCertificate -PolicyPath $testPolicyPath -Data $testPkcs12Data -PrivateKeyPassword $cred
            Should -Invoke -CommandName 'ConvertTo-PlaintextString' -Times 1 -ModuleName $ModuleName
        }
    ...
    Context 'Reconcile' {
        ...
        It 'Should set Reconcile in body when switch is used' {
            Import-CmCertificate -PolicyPath $testPolicyPath -Data $testPkcs12Data -Reconcile
            Should -Invoke -CommandName 'Invoke-TrustParallel' -Times 1 -ModuleName $ModuleName -ParameterFilter {
    ...
    Context 'Name parameter' {
        ...
        It 'Should set ObjectName when -Name is provided' {
            Import-CmCertificate -PolicyPath $testPolicyPath -Data $testPkcs12Data -Name 'MyCert'
            Should -Invoke -CommandName 'Invoke-TrustParallel' -Times 1 -ModuleName $ModuleName -ParameterFilter {
    ...
    Context 'EnrollmentAttribute' {
        ...
        It 'Should set CASpecificAttributes when EnrollmentAttribute is provided' {
            ...
            Import-CmCertificate -PolicyPath $testPolicyPath -Data $testPkcs12Data -EnrollmentAttribute $attrs
            Should -Invoke -CommandName 'Invoke-TrustParallel' -Times 1 -ModuleName $ModuleName -ParameterFilter {
    ...
    Context 'Policy path handling' {
        ...
        It 'Should call ConvertTo-CmFullPath to normalize the path' {
            Import-CmCertificate -PolicyPath 'certificates' -Data $testPkcs12Data
            Should -Invoke -CommandName 'ConvertTo-CmFullPath' -Times 1 -ModuleName $ModuleName
        }

        It 'Should check if the policy path exists' {
            Import-CmCertificate -PolicyPath $testPolicyPath -Data $testPkcs12Data
            Should -Invoke -CommandName 'Test-CmObject' -Times 1 -ModuleName $ModuleName -ParameterFilter {
                $Path -eq $testPolicyPath -and $ExistOnly -eq $true
            }
        }

        It 'Should write error when policy path does not exist without Force' {
            Mock -CommandName 'Test-CmObject' -MockWith { $false } -ModuleName $ModuleName
            Import-CmCertificate -PolicyPath $testPolicyPath -Data $testPkcs12Data -ErrorVariable err -ErrorAction SilentlyContinue
        ...
        It 'Should create policy path when Force is used and path does not exist' {
            Mock -CommandName 'Test-CmObject' -MockWith { $false } -ModuleName $ModuleName
            Mock -CommandName 'New-CmPolicy' -MockWith {} -ModuleName $ModuleName
            Import-CmCertificate -PolicyPath $testPolicyPath -Data $testPkcs12Data -Force
            Should -Invoke -CommandName 'New-CmPolicy' -Times 1 -ModuleName $ModuleName -ParameterFilter {
        ...
        It 'Should not create policy path when it already exists' {
            Mock -CommandName 'New-CmPolicy' -MockWith {} -ModuleName $ModuleName
            Import-CmCertificate -PolicyPath $testPolicyPath -Data $testPkcs12Data -Force
            Should -Invoke -CommandName 'New-CmPolicy' -Times 0 -ModuleName $ModuleName
        }
    }
    ...
    Context 'ThrottleLimit' {
        ...
        It 'Should pass ThrottleLimit to Invoke-TrustParallel' {
            Import-CmCertificate -PolicyPath $testPolicyPath -Data $testPkcs12Data -ThrottleLimit 5
            Should -Invoke -CommandName 'Invoke-TrustParallel' -Times 1 -ModuleName $ModuleName -ParameterFilter {
                $ThrottleLimit -eq 5
            }
        }
    ...
    Context 'Pipeline input' {
        ...
        It 'Should accept CertificateData alias' {
            [pscustomobject]@{ CertificateData = $testPkcs12Data; PolicyPath = $testPolicyPath } | Import-CmCertificate
            Should -Invoke -CommandName 'Invoke-TrustParallel' -Times 1 -ModuleName $ModuleName -ParameterFilter {
                $InputObject[0].Data -eq $testPkcs12Data
            }
        }
