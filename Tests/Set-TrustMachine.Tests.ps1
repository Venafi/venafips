...
Describe 'Set-TrustMachine' -Tags 'Unit' {

    BeforeEach {
        Mock -CommandName 'Get-TrustMachine'         -MockWith { $mockMachine } -ModuleName $ModuleName
        Mock -CommandName 'Get-TrustData' -ParameterFilter { $Type -eq 'VSatellite' } -MockWith { $testSatelliteId } -ModuleName $ModuleName
    ...
    Context 'Machine not found' {
        ...
        It 'Should write an error when machine is not found' {
            Mock -CommandName 'Get-TrustMachine' -MockWith { $null } -ModuleName $ModuleName
            Set-TrustMachine -Machine 'missing-machine' -Name 'NewName' -ErrorVariable err -ErrorAction SilentlyContinue
            $err | Should -Not -BeNullOrEmpty
        }

        It 'Should not call Invoke-TrustRestMethod when machine not found' {
            Mock -CommandName 'Get-TrustMachine' -MockWith { $null } -ModuleName $ModuleName
            Set-TrustMachine -Machine 'missing-machine' -Name 'NewName' -ErrorAction SilentlyContinue
    ...
    Context 'No update parameters' {
        ...
        It 'Should write an error when no updatable parameters are provided' {
            Set-TrustMachine -Machine $testMachineId -ErrorVariable err -ErrorAction SilentlyContinue
            $err | Should -Not -BeNullOrEmpty
        }

        It 'Should not call Invoke-TrustRestMethod when nothing to update' {
            Set-TrustMachine -Machine $testMachineId -ErrorAction SilentlyContinue
            Should -Invoke -CommandName 'Invoke-TrustRestMethod' -Times 0 -ModuleName $ModuleName
        }
    }
    ...
    Context '-Name update' {
        ...
        It 'Should call PATCH on machines/{id}' {
            Set-TrustMachine -Machine $testMachineId -Name 'NewName' -Confirm:$false
    ...
    Context '-ConnectionDetail update' {
        ...
        It 'Should call PATCH with connectionDetails in the body' {
            Set-TrustMachine -Machine $testMachineId -ConnectionDetail @{ hostnameOrAddress = 'new-host.example.com' } -Confirm:$false
            Should -Invoke -CommandName 'Invoke-TrustRestMethod' -Times 1 -ModuleName $ModuleName -ParameterFilter {
        ...
        It 'Should deep-merge nested connection detail hashtable' {
            Set-TrustMachine -Machine $testMachineId -ConnectionDetail @{ kerberos = @{ keyDistributionCenter = 'new-kdc.example.com' } } -Confirm:$false
            Should -Invoke -CommandName 'Invoke-TrustRestMethod' -Times 1 -ModuleName $ModuleName -ParameterFilter {
    ...
    Context '-Satellite update' {
        ...
        It 'Should look up satellite ID via Get-TrustData' {
            Set-TrustMachine -Machine $testMachineId -Satellite 'MySat' -Confirm:$false
    ...
    Context '-PassThru' {
        ...
        It 'Should not return output without PassThru' {
            $result = Set-TrustMachine -Machine $testMachineId -Name 'NewName' -Confirm:$false
            $result | Should -BeNullOrEmpty
        }

        It 'Should return updated machine with PassThru' {
            Mock -CommandName 'Get-TrustMachine' -MockWith { $mockUpdatedMachine } -ModuleName $ModuleName -ParameterFilter { $Machine -eq $testMachineId }
            $result = Set-TrustMachine -Machine $testMachineId -Name 'NewName' -PassThru -Confirm:$false
    ...
    Context 'Pipeline input' {
        ...
        It 'Should accept machineId from pipeline' {
            [pscustomobject]@{ machineId = $testMachineId } | Set-TrustMachine -Name 'PipelineName' -Confirm:$false
            Should -Invoke -CommandName 'Get-TrustMachine' -Times 1 -ModuleName $ModuleName
        }
    }
}
