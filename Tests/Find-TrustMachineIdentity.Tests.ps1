...
Describe 'Find-TrustMachineIdentity' -Tags 'Unit' {

    BeforeEach {
        Mock -CommandName 'Find-TrustObject'      -MockWith { $mockMachineIdentity } -ModuleName $ModuleName
        Mock -CommandName 'Get-TrustData' -ParameterFilter { $Type -eq 'Machine' } -MockWith { $testMachineId } -ModuleName $ModuleName
    ...
    Context 'No filter - get all' {
        ...
        It 'Should call Find-TrustObject with type MachineIdentity' {
            Find-TrustMachineIdentity
            Should -Invoke -CommandName 'Find-TrustObject' -Times 1 -ModuleName $ModuleName -ParameterFilter {
    ...
    Context '-Status filter' {
        ...
        It 'Should pass a single status filter without AND prefix' {
            Find-TrustMachineIdentity -Status INSTALLED
            Should -Invoke -CommandName 'Find-TrustObject' -Times 1 -ModuleName $ModuleName -ParameterFilter {
                $Filter -ne $null -and
        ...
        It 'Should use MATCH operator for status' {
            Find-TrustMachineIdentity -Status DISCOVERED
            Should -Invoke -CommandName 'Find-TrustObject' -Times 1 -ModuleName $ModuleName -ParameterFilter {
                $Filter[0][1] -eq 'MATCH'
            }
        }
        ...
        It 'Should uppercase the status value' {
            Find-TrustMachineIdentity -Status VALIDATED
            Should -Invoke -CommandName 'Find-TrustObject' -Times 1 -ModuleName $ModuleName -ParameterFilter {
    ...
    Context '-Machine filter' {
        ...
        It 'Should look up machine ID via Get-TrustData' {
            Find-TrustMachineIdentity -Machine 'my-machine'
    ...
    Context '-Certificate filter' {
        ...
        It 'Should look up certificate ID via Get-TrustData' {
            Find-TrustMachineIdentity -Certificate 'my-cert'
            Should -Invoke -CommandName 'Get-TrustData' -Times 1 -ModuleName $ModuleName -ParameterFilter {
    ...
    Context 'Multiple filters - two clauses passed to Find-TrustObject' {

        It 'Should pass two filter clauses when both Status and Machine are provided' {
            Find-TrustMachineIdentity -Status INSTALLED -Machine 'my-machine'
            Should -Invoke -CommandName 'Find-TrustObject' -Times 1 -ModuleName $ModuleName -ParameterFilter {
                $Filter.Count -eq 2
            }
        }
        ...
        It 'Should pass two filter clauses when Status and Certificate are provided' {
            Find-TrustMachineIdentity -Status DISCOVERED -Certificate 'my-cert'
            Should -Invoke -CommandName 'Find-TrustObject' -Times 1 -ModuleName $ModuleName -ParameterFilter {
                $Filter.Count -eq 2
            }
        }
        ...
        It 'Should pass two filter clauses when Machine and Certificate are provided' {
            Find-TrustMachineIdentity -Machine 'my-machine' -Certificate 'my-cert'
            Should -Invoke -CommandName 'Find-TrustObject' -Times 1 -ModuleName $ModuleName -ParameterFilter {
                $Filter.Count -eq 2
            }
        }
    ...
    Context '-Filter parameter set' {
        ...
        It 'Should pass the provided filter directly to Find-TrustObject' {
            $customFilter = [System.Collections.Generic.List[object]]@('AND', @('status', 'EQ', 'INSTALLED'))
            Find-TrustMachineIdentity -Filter $customFilter
            Should -Invoke -CommandName 'Find-TrustObject' -Times 1 -ModuleName $ModuleName -ParameterFilter {
    ...
    Context '-First parameter' {
        ...
        It 'Should pass First to Find-TrustObject' {
            Find-TrustMachineIdentity -First 5
            Should -Invoke -CommandName 'Find-TrustObject' -Times 1 -ModuleName $ModuleName -ParameterFilter {
                $First -eq 5
            }
        }
    ...
    Context 'Output' {
        ...
        It 'Should return results from Find-TrustObject' {
            $result = Find-TrustMachineIdentity
