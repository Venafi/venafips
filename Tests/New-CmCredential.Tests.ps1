...
Describe 'New-CmCredential' -Tags 'Unit' {
    ...
    Context 'Password Credential with String' {
        ...
        It 'Should create a Password credential' {
            New-CmCredential -Path $testPath -Secret $testPassword -Confirm:$false
        ...
        It 'Should not return output without PassThru' {
            $result = New-CmCredential -Path $testPath -Secret $testPassword -Confirm:$false
            $result | Should -BeNullOrEmpty
        }

        It 'Should return object with PassThru' {
            $result = New-CmCredential -Path $testPath -Secret $testPassword -PassThru -Confirm:$false
            $result | Should -Not -BeNullOrEmpty
            Should -Invoke -CommandName 'Get-CmObject' -Times 1 -ModuleName $ModuleName
        }
    }
    ...
    Context 'Password Credential with SecureString' {
        ...
        It 'Should create a Password credential' {
            New-CmCredential -Path $testPath -Secret $securePassword -Confirm:$false
            Should -Invoke -CommandName 'Invoke-TrustRestMethod' -Times 1 -ModuleName $ModuleName -ParameterFilter {
                $Body.FriendlyName -eq 'Password'
            }
        }
    ...
    Context 'Username Password Credential with PSCredential' {
        ...
        It 'Should create a UsernamePassword credential' {
            New-CmCredential -Path $testPath -Secret $psCred -Confirm:$false
            Should -Invoke -CommandName 'Invoke-TrustRestMethod' -Times 1 -ModuleName $ModuleName -ParameterFilter {
    ...
    Context 'Certificate Credential' {
        ...
        BeforeAll {
            ...
            $tempCertPath = Join-Path ([System.IO.Path]::GetTempPath()) 'test-cm-cred.pfx'
        ...
        It 'Should create a Certificate credential' {
            New-CmCredential -Path $testPath -Secret $certPassword -CertificatePath $tempCertPath -Confirm:$false
            Should -Invoke -CommandName 'Invoke-TrustRestMethod' -Times 1 -ModuleName $ModuleName -ParameterFilter {
        ...
        It 'Should not include Username in Values' {
            ...
            New-CmCredential -Path $testPath -Secret $psCred -CertificatePath $tempCertPath -Confirm:$false
            Should -Invoke -CommandName 'Invoke-TrustRestMethod' -Times 1 -ModuleName $ModuleName -ParameterFilter {
        ...
        It 'Should throw on invalid certificate path' {
            { New-CmCredential -Path $testPath -Secret $certPassword -CertificatePath 'C:\nonexistent.pfx' -Confirm:$false } |
                Should -Throw
        }

        It 'Should throw on wrong certificate password' {
            { New-CmCredential -Path $testPath -Secret 'wrongpassword' -CertificatePath $tempCertPath -Confirm:$false } |
    ...
    Context 'Path handling' {
        ...
        It 'Should call ConvertTo-CmFullPath with the provided path' {
            New-CmCredential -Path $testPath -Secret $testPassword -Confirm:$false
