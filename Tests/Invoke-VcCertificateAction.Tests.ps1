...
Describe 'Invoke-VcCertificateAction' -Tags 'Unit' {
    ...
    Context 'Retire' {
        ...
        It 'Should return success status' {
            $result = Invoke-VcCertificateAction -ID $testCertId -Retire -Confirm:$false
            $result.CertificateID | Should -Be $testCertId
            $result.Success | Should -BeTrue
        }

        It 'Should support pipeline input' {
            $result = [guid]$testCertId | Invoke-VcCertificateAction -Retire -Confirm:$false
    ...
    Context 'Renew' {
        ...
        It 'Should return renewal details on success' {
            $result = Invoke-VcCertificateAction -ID $testCertId -Renew -Confirm:$false
        ...
        It 'Should fail for non-CURRENT certificates' {
            Mock -CommandName 'Get-VcCertificate' -MockWith { $mockCertOldVersion } -ModuleName $ModuleName
            $result = Invoke-VcCertificateAction -ID $testCertId -Renew -Confirm:$false
        ...
        It 'Should fail with multiple CNs without Force' {
            Mock -CommandName 'Get-VcCertificate' -MockWith { $mockCertMultipleCN } -ModuleName $ModuleName
            $result = Invoke-VcCertificateAction -ID $testCertId -Renew -Confirm:$false
            $result.success | Should -BeFalse
        ...
        It 'Should succeed with multiple CNs when Force is used' {
            Mock -CommandName 'Get-VcCertificate' -MockWith { $mockCertMultipleCN } -ModuleName $ModuleName
            $result = Invoke-VcCertificateAction -ID $testCertId -Renew -Force -Confirm:$false
            $result.success | Should -BeTrue
        }

        It 'Should use provided Application parameter' {
            Invoke-VcCertificateAction -ID $testCertId -Renew -Application $testAppId -Confirm:$false
            Should -Invoke -CommandName 'Get-VcData' -ModuleName $ModuleName -ParameterFilter {
                $Type -eq 'Application' -and $Object -eq $true
            }
        }

        It 'Should use provided IssuingTemplate parameter' {
            Invoke-VcCertificateAction -ID $testCertId -Renew -IssuingTemplate $testTemplateId -Confirm:$false
            Should -Invoke -CommandName 'Get-VcData' -ModuleName $ModuleName -ParameterFilter {
    ...
    Context 'Revoke' {
        ...
        It 'Should call GraphQL revoke mutation' {
            Invoke-VcCertificateAction -ID $testCertId -Revoke -Confirm:$false
            Should -Invoke -CommandName 'Invoke-VcGraphQL' -Times 1 -ModuleName $ModuleName
        }

        It 'Should use default reason UNSPECIFIED' {
            Invoke-VcCertificateAction -ID $testCertId -Revoke -Confirm:$false
            Should -Invoke -CommandName 'Invoke-VcGraphQL' -Times 1 -ModuleName $ModuleName -ParameterFilter {
                $Variables.revocationReason -eq 'UNSPECIFIED'
            }
        }

        It 'Should accept a custom reason' {
            Invoke-VcCertificateAction -ID $testCertId -Revoke -Reason KEY_COMPROMISE -Confirm:$false
            Should -Invoke -CommandName 'Invoke-VcGraphQL' -Times 1 -ModuleName $ModuleName -ParameterFilter {
        ...
        It 'Should accept a custom comment' {
            Invoke-VcCertificateAction -ID $testCertId -Revoke -Comment 'test revoke' -Confirm:$false
            Should -Invoke -CommandName 'Invoke-VcGraphQL' -Times 1 -ModuleName $ModuleName -ParameterFilter {
        ...
        It 'Should return success on revoke' {
            $result = Invoke-VcCertificateAction -ID $testCertId -Revoke -Confirm:$false
            $result.success | Should -BeTrue
            $result.CertificateId | Should -Be $testCertId
        }

        It 'Should return error on failure' {
            Mock -CommandName 'Invoke-VcGraphQL' -MockWith { throw 'Revoke failed' } -ModuleName $ModuleName
            $result = Invoke-VcCertificateAction -ID $testCertId -Revoke -Confirm:$false
            $result.success | Should -BeFalse
    ...
    Context 'Delete' {
        ...
        BeforeEach {
            Mock -CommandName 'Invoke-VcCertificateAction' -MockWith {} -ModuleName $ModuleName
        }
    ...
    Context 'Provision' {
        ...
        BeforeEach {
            Mock -CommandName 'Find-VcMachineIdentity' -MockWith {
                [pscustomobject]@{ machineIdentityId = 'mi-123' }
            } -ModuleName $ModuleName
            Mock -CommandName 'Invoke-VcWorkflow' -MockWith {} -ModuleName $ModuleName
        }

        It 'Should find machine identities and invoke workflow' {
            Invoke-VcCertificateAction -ID $testCertId -Provision -Confirm:$false
            Should -Invoke -CommandName 'Find-VcMachineIdentity' -Times 1 -ModuleName $ModuleName
        ...
        It 'Should throw when no machine identities found' {
            Mock -CommandName 'Find-VcMachineIdentity' -MockWith {} -ModuleName $ModuleName
            { Invoke-VcCertificateAction -ID $testCertId -Provision -Confirm:$false } | Should -Throw '*No machine identities*'
        }
    }
    ...
    Context 'Provision to CloudKeystore' {
        ...
        It 'Should call GraphQL provision mutation' {
            Invoke-VcCertificateAction -ID $testCertId -Provision -CloudKeystore 'my-keystore' -Confirm:$false
            Should -Invoke -CommandName 'Invoke-VcGraphQL' -Times 1 -ModuleName $ModuleName
        }

        It 'Should return success' {
            $result = Invoke-VcCertificateAction -ID $testCertId -Provision -CloudKeystore 'my-keystore' -Confirm:$false
            $result.success | Should -BeTrue
            $result.certificateId | Should -Be $testCertId
        }
    }
