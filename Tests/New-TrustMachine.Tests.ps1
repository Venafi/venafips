...
Describe 'New-TrustMachine' -Tags 'Unit' {

    BeforeEach {
        Mock -CommandName 'Initialize-PSSodium' -MockWith {} -ModuleName $ModuleName
        Mock -CommandName 'Get-TrustData' -ParameterFilter { $Type -eq 'Plugin' } -MockWith { $mockPlugin } -ModuleName $ModuleName
        Mock -CommandName 'Get-TrustData' -ParameterFilter { $Type -eq 'Team' } -MockWith { $testTeamId } -ModuleName $ModuleName
        ...
        # Mock Invoke-TrustParallel to avoid needing Invoke-TrustRestMethod/Invoke-TrustWorkflow inside scriptblock
    ...
    Context 'Basic machine creation' {
        ...
        It 'Should call Invoke-TrustParallel with TrustClient' {
            ...
            New-TrustMachine -Name 'c1' -MachineType 'Citrix ADC' -Owner 'MyTeam' -Credential $cred
    ...
    Context 'PassThru' {
        ...
        It 'Should not return output without PassThru' {
            $cred = New-Object PSCredential('user', ('pass' | ConvertTo-SecureString -AsPlainText -Force))
            $result = New-TrustMachine -Name 'c1' -MachineType 'Citrix ADC' -Owner 'MyTeam' -Credential $cred
            $result | Should -BeNullOrEmpty
        }

        It 'Should return machine details with PassThru' {
            $cred = New-Object PSCredential('user', ('pass' | ConvertTo-SecureString -AsPlainText -Force))
            $result = New-TrustMachine -Name 'c1' -MachineType 'Citrix ADC' -Owner 'MyTeam' -Credential $cred -PassThru
    ...
    Context 'Invalid input' {
        ...
        It 'Should error on invalid machine type' {
            Mock -CommandName 'Get-TrustData' -ParameterFilter { $Type -eq 'Plugin' } -MockWith { $null } -ModuleName $ModuleName
            $cred = New-Object PSCredential('user', ('pass' | ConvertTo-SecureString -AsPlainText -Force))
            New-TrustMachine -Name 'c1' -MachineType 'BadType' -Owner 'MyTeam' -Credential $cred -ErrorVariable err -ErrorAction SilentlyContinue
            $err | Should -Not -BeNullOrEmpty
        }

        It 'Should error on invalid owner' {
            Mock -CommandName 'Get-TrustData' -ParameterFilter { $Type -eq 'Team' } -MockWith { $null } -ModuleName $ModuleName
            $cred = New-Object PSCredential('user', ('pass' | ConvertTo-SecureString -AsPlainText -Force))
            New-TrustMachine -Name 'c1' -MachineType 'Citrix ADC' -Owner 'BadTeam' -Credential $cred -ErrorVariable err -ErrorAction SilentlyContinue
            $err | Should -Not -BeNullOrEmpty
        }
    }
